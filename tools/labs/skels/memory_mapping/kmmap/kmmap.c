		...
		size_t size, loff_t *offset)
{
	...
	err = copy_to_user((void *)user_buffer, kmalloc_area, size);
	if (err)
		return -1;
		...
		size_t size, loff_t *offset)
{
	...
	err = copy_from_user((void *)user_buffer, kmalloc_area, size);
...
static int my_seq_show(struct seq_file *seq, void *v)
{
	...
	do {
		...
		seq_printf(seq, "(%ld : %ld)", vma_iterator->vm_start, vma_iterator->vm_end);
...
static int __init my_init(void)
{
	...
	procfs_entry = proc_create(PROC_ENTRY_NAME, 0, NULL, &my_proc_ops);
	if (!procfs_entry) {
	...
	if (kmalloc_ptr == NULL) {
		pr_info("Error at kmalloc for pages\n");
		return -1;
	}
	...
	for (i = 0; i < NPAGES * PAGE_SIZE; i+= PAGE_SIZE) {
		memset(kmalloc_area, 0xaa, 1);
		memset(kmalloc_area + 1, 0xbb, 1);
		memset(kmalloc_area + 2, 0xcc, 1);
		memset(kmalloc_area + 3, 0xdd, 1);
	}
...
static void __exit my_exit(void)
{
	...
	for(i = 0; i < NPAGES * PAGE_SIZE; i += PAGE_SIZE)
        ClearPageReserved(virt_to_page(((unsigned long)kmalloc_ptr) + i));
